...
class Variogram_on_Grid:
    ...
    def __init__(self, nx, ny, nz, reals, n_lags, step, exhaust=None):
        self.nx = nx
        self.ny = ny
        self.nz = nz
        self.reals = reals
    ...
    def calculate(self):
        ...
        variances_vert_list = []
        variances_horiz_exhaust = []
        variances_vert_exhaust = []
        ...
        for lag in self.lag_lst:
            ...
            if self.exhaust is not None:
                values = get_values(pairs_in_horiz, self.exhaust)
                variance_val = 1/2 * np.nanmean([variance(v) for v in values])
                variances_horiz_exhaust.append(variance_val)
            
                if self.nz > 1:
                    values_v = get_values(pairs_in_vert, self.exhaust)
                    variance_val_v = 1/2 * np.nanmean([variance(v) for v in values_v])
                    variances_vert_exhaust.append(variance_val_v)
        ...
        self.variances_horiz_exhaust = variances_horiz_exhaust
        self.variances_vert_exhaust = variances_vert_exhaust
    ...
    def plot(self, horiz_block, vert_block, model):
        ...
        if self.nz == 1:
            ...
            if self.exhaust is not None:
                axes.plot(x_axis_h, self.variances_horiz_exhaust, color='blue')
            ...
            if self.exhaust is not None:
                axes[0].plot(x_axis_h, self.variances_horiz_exhaust, color='blue')
                axes[1].plot(x_axis_h, self.variances_vert_exhaust, color='blue')
